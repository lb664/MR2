# 135 characters #####################################################################################################################
#' @title False Discovery Rate based on Beta densities finite mixture model
#' @description Post-processing of MR2 Markov chain Monte Carlo output. Estimation of the posterior probability of inclusion cutoff 
#' for a given nominal False Discovery Rate level
#'
#' @param PPI Ordered marginal posterior probability of inclusion (mPPI) or edge marginal posterior of inclusion (ePPI) return by 
#' MBB_EM()
#' @param w Matrix of allocation probabilities
#' @param FDR_nominal Nominal level of False Discovery Rate to be controlled. Default values set at \code{0.05}
#' @param plotting Logical with no plots as default \code{FALSE}
#'
#' @details
...
#' @export
#'
#' @return The value returned is a list object \code{list(cutoff, cutoff_idx, FDR_est, FNR_est)}
#' \itemize{
...
FDR_BMM <- function(PPI, w, FDR_nominal = 0.05, plotting = FALSE)
{
    N <- length(PPI)
    K <- ncol(w)
    ...
    if (sum((cumsum(w_FPR) / seq(1, N)) < FDR_nominal) > 0)
    {
        cutoff_idx <- max(which((cumsum(w_FPR) / seq(1, N)) < FDR_nominal))
        cutoff <- PPI[seq(N, 1)][cutoff_idx]
        
        FDR_est <- mean(w_FPR[1 : cutoff_idx])
        FNR_est <- mean(w_FNR[(cutoff_idx + 1) : N])
    } else {
    ...
    if (plotting == TRUE & sum((cumsum(w_FPR) / seq(1, N)) < FDR_nominal) > 0)
    {   
        plot(PPI[seq(N, 1)], (cumsum(w_FPR) / seq(1, N)), xaxt = 'n', xlim = c(0, 1), ylim = c(0, 1), 
             cex.lab = 1, cex.main = 1, xlab = "PPI", ylab = "Nominal and estimated FDR", main = "Local and global FDR")
        axis(1, at = seq(0, 1, length.out = 5), labels = seq(0, 1, length.out = 5))
        abline(h = FDR_nominal, lty = 2, lwd = 2, col = "red")
        abline(v = cutoff, lty = 2, lwd = 2, col = "red")
    }
    
    opt <- list(FDR_nominal = FDR_nominal, plotting = plotting)
