...
PostProc <- function(output, Y, X, alpha = 0.05, digits = 3)
{

  Y <- as.matrix(Y)
  X <- as.matrix(X)
  
  if (output$opt$std == TRUE)
  {
  ...
  gammaPost <- round(matrix(gammaPost, nrow = p, ncol = m, dimnames = list(colnames_X, colnames_Y)), digits = digits)
  thetaPost <- apply(output$theta, 2, mean)
  thetaPost <- round(matrix(thetaPost, nrow = p, ncol = m, dimnames = list(colnames_X, colnames_Y)), digits = digits)
  thetaPost_LL <- apply(output$theta, 2, quantile, alpha / 2)
  thetaPost_LL <- round(matrix(thetaPost_LL, nrow = p, ncol = m), digits = digits)
  thetaPost_UL <- apply(output$theta, 2, quantile, 1 - alpha / 2)
  thetaPost_UL <- round(matrix(thetaPost_UL, nrow = p, ncol = m), digits = digits)
  thetaPost_CI <- array(c(thetaPost_LL, thetaPost_UL), dim = c(p, m , 2), dimnames = list(colnames_X, colnames_Y, c(alpha / 2, 1 - alpha / 2)))
  DPost <- round(colMeans(output$D), digits = digits)
  
  GPost <- round(apply(output$G, c(1, 2), mean), digits = digits)
  ...
  SigmaPost <- round(matrix(colMeans(SigmaPost_TMP), nrow = m, ncol = m), digits = digits)
  SigmaPost_LL <- apply(SigmaPost_TMP, 2, quantile, alpha / 2)
  SigmaPost_LL <- round(matrix(SigmaPost_LL, nrow = m, ncol = m), digits = digits)
  SigmaPost_LL[upper.tri(SigmaPost_LL, diag = FALSE)] <- NA
  SigmaPost_UL <- apply(SigmaPost_TMP, 2, quantile, 1 - alpha / 2)
  SigmaPost_UL <- round(matrix(SigmaPost_UL, nrow = m, ncol = m), digits = digits)
  ...
  Rm1Post <- round(matrix(colMeans(Rm1Post_TMP), nrow = m, ncol = m), digits = digits)
  Rm1Post[upper.tri(Rm1Post, diag = TRUE)] <- NA
  Rm1Post_LL <- apply(Rm1Post_TMP, 2, quantile, alpha / 2)
  Rm1Post_LL <- round(matrix(Rm1Post_LL, nrow = m, ncol = m), digits = digits)
  Rm1Post_LL[upper.tri(Rm1Post_LL, diag = TRUE)] <- NA
  Rm1Post_UL <- apply(Rm1Post_TMP, 2, quantile, 1 - alpha / 2)
  Rm1Post_UL <- round(matrix(Rm1Post_UL, nrow = m, ncol = m), digits = digits)
  ...
  for (iter in 1 : niter)
  {
    ...
    if ((iter %% 100) == 0)
    {
      cat("Checking decomposability:", round(iter / niter, digits = digits), "\n")
    }
  ...
  for (iter in 1 : niter)
  {
    ...
    if ((iter %% 100) == 0)
    {
      cat("Computing PLML:", round(iter / niter, digits = digits), "\n")
    }
  ...
  scaleCPO <- round(CPO / apply(f_i, 1, max), digits = digits)
  PLML <- round(sum(log(CPO)), digits = digits)
  mPLML <- mean(log(CPO))
  stdPLML <- var(log(CPO)) ^(1 /2)
  
  idx <- which(scaleCPO < 0.01)
  freq_ouliers <- round(length(idx) / length(CPO), digits = digits)
  stability_PLML <- round(sum(log(CPO) < -10) / length(CPO), digits = digits)
  ...
  D_bar <- -2 * sum(log(f_i))
  pD <- bar_D - D_bar
  DIC <- round(2 * bar_D - D_bar, digits = digits)
  
  SigmaPost[upper.tri(SigmaPost, diag = FALSE)] <- NA
  CPO <- round(CPO, digits = digits)
  scaleCPO <- round(scaleCPO, digits = digits) 
  
  opt = list(alpha = alpha, std = output$opt$std, digits = digits)
