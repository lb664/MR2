                               ...
                               thetaUY = 1, thetaUX = 2, seed = 31122021)
{
  ...
  for (Rep in 1 : nRep)
  {
    ...
    if (undirectedA == TRUE)
    {
      betaA <- runif(n = nIV, min = betaHatRange_X[1], max = betaHatRange_X[2])
    } else {
      betaA <- runif(n = nIV, min = 0, max = betaHatRange_X[2])
    }
  ...
  return(list(betaHat_Y = betaHat_YAll, betaHat_X = betaHat_XAll, theta = thetaAll, mediation = medAll, par = par))
}
