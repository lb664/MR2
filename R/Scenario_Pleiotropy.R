# 135 characters #####################################################################################################################
#' @title Simulated scenario: Pleiotropy
...
#'         thetaSparsness, heritY, betaXRange, heritX, rhoX, undirectedA, thetaA, thetaUY, thetaUX, std, seed)} } }
#'
#' @references
#' \insertAllCited{}
#'
...
Scenario_Pleiotropy <- function(nRep = 2, q = 5, p = 10, nIV = 100, nSubject = 50000, MAF = 0.05,      
                                thetaRange = c(-2, 2), thetaSparsness = 0.30, heritY = 0.25, 
                                betaXRange = c(-2, 2), heritX = 0.10, rhoX = 0.60, 
                                undirectedA = TRUE, thetaA = 1, 
                                thetaUY = 1, thetaUX = 2, std = TRUE, seed = 31122021)
{
  ...
  par = list(nRep = nRep, q = q, p = p, nIV = nIV, nSubject = nSubject, MAF = MAF, 
             ...
             undirectedA = undirectedA, thetaA = thetaA, 
             betaXRange = betaXRange, heritX = heritX, rhoX = rhoX, 
             thetaUY = thetaUY, thetaUX = thetaUX, std = FALSE, seed = seed)
  
  return(list(Y = YAll, X = XAll, theta = thetaAll, par = par))
}
